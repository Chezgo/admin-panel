  ...
  <div class="page">
    ...
    <div v-if="isNew" class="card detail-card">
      ...
      <form @submit.prevent="submitCreate" class="form">
        <div class="form-group">
          <label>Название *</label>
          <input v-model="createForm.name" required placeholder="Например: Levenhuk">
        </div>
...
const brandId = computed(() => isNew.value ? null : parseInt(route.params.id, 10));
...
const fetchBrand = async () => {
  ...
  error.value = null;
  try {
    const res = await brandsApi.getById(brandId.value);
    brand.value = res.data;
  } catch (err) {
    error.value = err.response?.data?.message || 'Не удалось загрузить бренд';
    console.error(err);
  } finally {
    loading.value = false;
  }
...
const handleDelete = async () => {
  if (!confirm(`Удалить бренд "${brand.value.name}"?`)) return;
